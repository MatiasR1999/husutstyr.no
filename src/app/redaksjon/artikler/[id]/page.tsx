...
export default async function DraftPage({params}:{params:Promise<{id:string}>}) {
 ...
 <p className="meta">{article.publishedRevisionId===article.revisionId?ui.flowPublished:article.approved?ui.flowApproved:article.submitted?ui.flowReview:ui.flowDraft}</p>
 <form method="post" action={`/api/editor/articles/${id}`} className="actions"><input type="hidden" name="revisionId" value={article.revisionId}/>{!article.submitted&&<button name="action" value="submit">{ui.submit}</button>}{session.principal.role==='editor'&&article.submitted&&!article.approved&&<button name="action" value="approve">{ui.approve}</button>}</form>
