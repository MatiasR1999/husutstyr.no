...
export async function ArticleView({article,now}:{article:PublishedArticle;now:number}) {
 ...
 const leadFigure=bilde?<figure className="article-lead"><Image src={bilde.url} alt={bilde.alt} width={bilde.bredde} height={bilde.hoyde} sizes={site.media.sizes} priority /><figcaption>{`${site.ui.photo}: ${bilde.fotograf} / ${site.ui.photoSource}`}</figcaption></figure>:null;
 ...
 return <article className={`stack article-${variant}`} data-layout={variant} data-publication-hash={article.contentHash}>
  ...
  {variant==='classic'?<>{heading}{contents}{body}</>:variant==='editorial'?<><div className="editorial-intro">{heading}<aside>{contents}</aside></div>{leadFigure}{body}</>:<>{heading}<div className="reference-grid"><aside className="stack">{byline}{contents}</aside>{body}</div></>}
