...
export function launchContentIssues(site:SiteDefinition,inventory:LaunchInventory):string[] {
 ...
 for(const [index,row] of inventory.articles.entries()) {
  ...
  try {
   ...
   if(author.is_test||!author.name.trim()||!author.bio.trim()||!author.image||!author.image.rights.trim()||!author.expertise?.length||!author.sameAs?.length)issues.push(`${key}:incomplete-author`);
