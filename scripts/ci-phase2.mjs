...
const runtime={...process.env,...parseEnv(await readFile('.env.phase2.local','utf8')),DATABASE_URL_UNPOOLED:'',QA_EVIDENCE_DIR:'docs/qa/phase2',NODE_OPTIONS:'--dns-result-order=ipv4first'};
const owner=parseEnv(await readFile('.env.test.local','utf8'));
const checks=[];
for(const [label,command,args,env] of [
 ['database',process.execPath,['--import','tsx','scripts/phase2/database-test.ts'],{...runtime,DATABASE_URL_UNPOOLED:owner.DATABASE_URL_UNPOOLED}],
 ...
 await writeFile(`docs/qa/phase2/${label}.log`,output);
 checks.push({label,command:[command,...args].join(' '),started,exitCode:code});
 await writeFile('docs/qa/phase2/checks.json',JSON.stringify(checks,null,2)+'\n');
